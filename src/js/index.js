...
(function($) {
  ...
  function liCreateLinkage(li_tag, header_level) {
    ...
    li_tag.click(function(e) {
      ...
      var eleId =
        main.content + " h" + header_level + "." + li_tag.attr("data-src");
      var header = $(eleId);
    ...
    title.map(function() {
      ...
      $(this).on("click", "a.section-link", function(event) {
        ...
        history.pushState(
          null,
          null,
          "#" + curName + "#" + replaceSymbols(content)
        );
  ...
  function router() {
    ...
    if (path === "" && main.index) path = main.index + ".md";
    else path = path + ".md";
